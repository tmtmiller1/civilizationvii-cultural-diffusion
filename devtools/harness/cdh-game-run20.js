// cdh-game-run20.js - game scope, deployed as ui/cdh-game.js. Cultural Diffusion harness run 18 (dev only).
//
...
import { wouldStrandForeignUnit, hasStrandableUnit } from "/cultural-diffusion/ui/cd-units.js";
...
async function buildGapPen(kinds) {
  ...
  emit(`G DIRECT hasStrandableUnit(centre)=${safe(() => hasStrandableUnit(centre, local), "ERR")} `
    + `wouldStrandForeignUnit(gap)=${safe(() => wouldStrandForeignUnit(theGap, local), "ERR")} `
    + `=> the pass must agree with this verdict`);
...
function gapState() {
  ...
  return { unitAt: at ? key(at) : "gone", gapOwner: owner(gap.loc),
    escapes: neighbours(here).filter((p) => landClean(p) && owner(p) !== local).length,
...
async function run() {
  ...
  emit(`S0 run20 turn=${safe(() => Game.turn)} local=${local} modPass=${CONFIG.diffusionEnabled} `
  ...
  emit(`G VERDICT centre=${key(gap.centre)} gap=${key(gap.loc)} unit=${gap.type} owner=${gap.unitOwner} `
    ...
    + (tookGap ? "FAILED: the pass took the gap the guard should have refused"
      : frozen ? "INCONCLUSIVE: gap held but the unit never moved anyway (AI may be idle)"
        : "HELD: the gap was never taken and the unit kept moving"));
}
...
engine.on("PlayerTurnActivated", (d) => {
  ...
  setTimeout(async () => {
    ...
    if ((gap && watched >= WATCH_TURNS) || n >= MAX_TURNS) { verdicts(); setTimeout(() => emit("DONE harness run20 finished"), 3000); return; }
