...
async function run() {
  ...
  emit(`S0 run23 turn=${safe(() => Game.turn)} local=${local} modPass=${CONFIG.diffusionEnabled} `
...
engine.on("PlayerTurnActivated", (d) => {
  ...
  setTimeout(async () => {
    ...
    if ((gap && watched >= WATCH_TURNS) || n >= MAX_TURNS) { verdicts(); setTimeout(() => emit("DONE harness run23 finished"), 3000); return; }
