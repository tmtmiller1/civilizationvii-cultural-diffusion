...
async function run() {
  ...
  emit(`S0 run29 turn=${safe(() => Game.turn)} local=${local} cities=${localCities().length}`);
  stageA();
  penDone = await stageN();
  emit("ACTIONS done; ending turns");
  setTimeout(endTurn, 6000);
}

// Ships move. Run 26/27 looked once, at turn 0, and found nothing in range; this keeps looking while the
// turns roll, exactly as run 17 had to for the Scout.
...
engine.on("PlayerTurnActivated", (d) => {
  ...
  setTimeout(async () => {
    ...
    if (!penDone) penDone = await stageN().catch((e) => { emit("stageN threw " + e); return false; });
    if (penDone || n >= 12) { setTimeout(() => emit("DONE harness run29 finished"), 3000); return; }
