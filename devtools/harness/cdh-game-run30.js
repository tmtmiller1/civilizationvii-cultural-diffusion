...
function run() {
  ...
  emit(`S0 run30 turn=${safe(() => Game.turn)} age=${ageName()} local=${local} `
...
engine.on("PlayerTurnActivated", (d) => {
  ...
  setTimeout(() => {
    ...
    if (n >= MAX_TURNS) { emit(`STRESS SURVIVED ${n} turns, final age ${age}`); setTimeout(() => emit("DONE harness run30 finished"), 3000); return; }
