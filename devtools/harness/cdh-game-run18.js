...
async function buildGapPen(kinds) {
  ...
  emit(`G PASS ${J(res)} gapOwner=${owner(theGap)} unitAt=${J(whereIsUnit(site.u.cid))} => `
    + (took ? "TOOK THE GAP (guard did NOT hold)" : "LEFT THE GAP (guard held)"));
  return true;
}
function gapState() {
  if (!gap) return null;
...
async function run() {
  ...
  emit(`S0 run18 turn=${safe(() => Game.turn)} local=${local} modPass=${CONFIG.diffusionEnabled} `
...
engine.on("PlayerTurnActivated", (d) => {
  ...
  setTimeout(async () => {
    ...
    if ((gap && watched >= WATCH_TURNS) || n >= MAX_TURNS) { verdicts(); setTimeout(() => emit("DONE harness run18 finished"), 3000); return; }
