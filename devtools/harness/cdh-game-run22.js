// cdh-game-run22.js - game scope, deployed as ui/cdh-game.js. Cultural Diffusion harness run 18 (dev only).
//
...
async function run() {
  ...
  emit(`S0 run22 turn=${safe(() => Game.turn)} local=${local} modPass=${CONFIG.diffusionEnabled} `
...
engine.on("PlayerTurnActivated", (d) => {
  ...
  setTimeout(async () => {
    ...
    if ((gap && watched >= WATCH_TURNS) || n >= MAX_TURNS) { verdicts(); setTimeout(() => emit("DONE harness run22 finished"), 3000); return; }
